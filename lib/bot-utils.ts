...
export function validateVariables(text: string): TokenValidation {
  ...
  const charMixed = charMatches.length - charLowercase.length - charUppercase.length
  ...
  const userMixed = userMatches.length - userLowercase.length - userUppercase.length
 ...
 */
export function characterCardToBot(card: CharacterCardV2): Omit<Bot, 'id' | 'createdAt' | 'updatedAt'> {
  const extensions = card.data.extensions?.janitorforge as { rating?: string } | undefined
 ...
 */
export async function importCharacterCardPNG(file: File): Promise<CharacterCardV2 | null> {
  return new Promise((resolve) => {
 ...
 */
export function generateReleasePost(bot: Bot, options: ReleasePostOptions): string {
  const { platform, includeStats, includePreview } = options
  
  const divider = platform === 'discord' ? '```' : '---'
  const bold = (text: string) => platform === 'reddit' ? `**${text}**` : `**${text}**`
  const italic = (text: string) => platform === 'reddit' ? `*${text}*` : `*${text}*`
  ...
  if (bot.tags.length > 0) {
    const tagFormat = bot.tags.map(tag => `#${tag.replace(/\s+/g, '')}`).join(' ')
    post += `${bold('Tags:')} ${tagFormat}\n\n`
  }
  
  // Scenario preview
  if (includePreview && bot.scenario) {
    post += `${bold('Scenario:')}\n`
    const previewScenario = bot.scenario.length > 200 
      ? bot.scenario.slice(0, 200) + '...' 
      : bot.scenario
  ...
  if (includeStats) {
    ...
    post += `- Created: ${bot.createdAt.toLocaleDateString()}\n\n`
  }
  
  // Footer
  post += `${divider}\n\n`
  post += `${italic('Created with JanitorForge')}\n`
