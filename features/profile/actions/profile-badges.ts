...
} from "@/features/profile/lib/profile-badges";
...
async function requireOwnerBadgeAccess() {
...
export async function listBadgeDefinitions(options?: {
  ...
  const { supabase, error } = await requireOwnerBadgeAccess();
  if (error)
...
export async function awardProfileBadge(input: AwardProfileBadgeInput) {
  const { supabase, user, error } = await requireOwnerBadgeAccess();
  ...
  if (!profileId || !badgeSlug) {
    return {
      success: false,
      error: "Profile id and badge slug are required",
    };
  }
