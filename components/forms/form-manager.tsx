...
function FormCard({ form, requestCount, onEdit, onDelete, onToggleActive, onCopyLink }: FormCardProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      ...
      <p className="mt-2 max-w-sm text-muted-foreground">
        Create your first request form to start collecting bot ideas from your community.
      </p>
...
export function FormManager() {
  const { forms, addForm, updateForm, deleteForm, getRequestsByFormId } = useStore()
  
  // UI State
  const [isCreating, setIsCreating] = useState(false)
  const [editingForm, setEditingForm] = useState<RequestForm | null>(null)
  const [deleteConfirmForm, setDeleteConfirmForm] = useState<RequestForm | null>(null)

  // Handlers
  const handleCreateForm = (formData: Omit<RequestForm, 'id' | 'shareableLink' | 'createdAt' | 'updatedAt'>) => {
    addForm(formData)
    setIsCreating(false)
    toast.success('Form created successfully!')
  }

  const handleUpdateForm = (formData: Omit<RequestForm, 'id' | 'shareableLink' | 'createdAt' | 'updatedAt'>) => {
    if (editingForm) {
  ...
  const handleToggleActive = (form: RequestForm) => {
    updateForm(form.id, { isActive: !form.isActive })
  ...
  return (
    <div className="p-6 lg:p-8">
      ...
      <Dialog open={!!deleteConfirmForm} onOpenChange={(open) => !open && setDeleteConfirmForm(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Delete Form</DialogTitle>
            <DialogDescription>
              Are you sure you want to delete &quot;{deleteConfirmForm?.title}&quot;? 
              This will also delete all associated requests. This action cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
